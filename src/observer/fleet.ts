...
export function globFiles(root: string, glob: string): string[] {
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    for (const d of dirs) {
      ...
      for (const name of entries) {
        if (!segMatches(seg, name)) continue
        const p = join(d, name)
        let st
...
function segMatches(seg: string, name: string): boolean {
  if (seg === '*') return true
...
export function readTenantEvents(stateDir: string, eventLogGlob = DEFAULT_GLOB): ObsEvent[] {
  ...
  for (const file of globFiles(stateDir, eventLogGlob)) {
    ...
    for (const line of body.split('\n')) {
      const t = line.trim()
      if (!t) continue
      try { events.push(JSON.parse(t) as ObsEvent) } catch { /* skip malformed */ }
    }
  }
  return events
}
 ...
 */
export function runFleet(registry: FleetRegistry, observerRunId: string, checkedAt: string | null = null): FindingsRecord[] {
