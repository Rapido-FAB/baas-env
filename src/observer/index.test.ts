...
describe('FA-OBSERVER core', () => {
  ...
  it('failure signatures: cos-capture fires on a cos downward command; a clean tenant is all-green', () => {
    const dirty: ObsEvent[] = [{ type: 'a2a.intent', source: 'cos', target: 'marketing', intent: 'delegate', direction: 'downward' }]
    const cosCapture = failureSignatures(dirty).find((s) => s.signature === 'cos-capture')!
    expect(cosCapture.present).toBe(true)

    const clean: ObsEvent[] = [{ type: 'a2a.intent', source: 'marketing', target: 'ceo', intent: 'request', direction: 'upward', allowListResult: 'pass' }]
  ...
  it('cadence verification: an expected rhythm that fired is ran-as-configured', () => {
    const events: ObsEvent[] = [{ type: 'routine.execution', routine: 'monthly-review', ts: '2026-05-01T10:00:00Z' }]
  ...
  it('FT-26/28 under-the-radar: the observer must never be a source/target of any tenant event', () => {
    const clean: ObsEvent[] = [{ type: 'a2a.intent', source: 'ceo', target: 'cos', intent: 'inform' }]
    expect(observerNeverParticipates(clean, 'observer')).toEqual([])
    const leaked: ObsEvent[] = [...clean, { type: 'a2a.intent', source: 'cos', target: 'observer', intent: 'inform' }]
